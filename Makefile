...
ifdef GITHUB_RUN_NUMBER
  FE_FLAGS += -DFE_BUILD_D='"( Build $(GITHUB_RUN_NUMBER) ) "'
  FE_FLAGS += -DFE_CACHE_VERSION_D=$(VER_MAJOR)$(VER_MINOR)$(VER_POINT)$(GITHUB_RUN_NUMBER)
else
  FE_FLAGS += -DFE_BUILD_D='""'
  FE_FLAGS += -DFE_CACHE_VERSION_D=$(VER_MAJOR)$(VER_MINOR)$(VER_POINT)
endif
...
$(RES_LANGUAGE_FILE): $(_LANGUAGE) $(RES_LANGUAGE_DIR)
	...
	$(shell (echo 'const std::map<const char*, const char*> _binary_languages = $(OPEN_BRACE)') > $(RES_LANGUAGE_FILE))
